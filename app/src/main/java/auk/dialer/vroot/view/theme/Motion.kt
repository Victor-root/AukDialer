...
object AukStaticMotion {
    ...
    fun <T> screenEnter(): FiniteAnimationSpec<T> =
        tween(AukDurations.Long2, easing = AukEasing.Emphasized)

    fun <T> screenExit(): FiniteAnimationSpec<T> =
        tween(AukDurations.Medium4, easing = AukEasing.EmphasizedAccelerate)

    fun <T> screenEnterEffects(): FiniteAnimationSpec<T> =
        tween(AukDurations.Medium4, easing = AukEasing.EmphasizedDecelerate)

    fun <T> screenExitEffects(): FiniteAnimationSpec<T> =
        tween(AukDurations.Short4, easing = AukEasing.EmphasizedAccelerate)

    fun <T> ambientLoop(durationMillis: Int): FiniteAnimationSpec<T> =
        tween(durationMillis, easing = AukEasing.Linear)
}
...
object AukMotion {
    ...
    @Composable
    fun <T> screenTransition(): FiniteAnimationSpec<T> = AukStaticMotion.screenEnter()

    @Composable
    fun <T> screenTransitionEffects(): FiniteAnimationSpec<T> = AukStaticMotion.screenEnterEffects()
